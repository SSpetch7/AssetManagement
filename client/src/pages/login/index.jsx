...
export default function Login() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        console.log('localhost:5000 one');
        if (res.data.Status === 'Success') {
          navigate('/home');
        } else {
          navigate('/login');
        ...
        >
          ...
          <div className="flex flex-col text-gray-500 py-2">
            ...
            <input
              ...
              name="admin_password"
              onChange={(e) =>
                setValues({ ...values, admin_password: e.target.value })
              }
            />
          </div>
          ...
          <button
            className="w-full my-2 py-2 bg-kmuttColor-800 shadow-lg text-white font-semibold rounded-lg"
